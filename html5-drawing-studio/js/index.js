console.clear();
console.log('HTML Drawing Studio v1.1.0', 'Check the HTML tab for a changelog');
...
var Brush = function(options)
{
	...
	$this.Draw = function(e)
	{
		if($this.LastPaint)
		{
			Artboard.Layer.Context.globalAlpha = $this.Opacity;
...
var Artboard = new (Artboard = function()
{
	...
	$this.ColorPickerCanvas = $('<canvas>')
		...
		.on(('ontouchstart' in document.documentElement) ? 'touchstart' : 'mousedown', function(e)
		{
			return $(this).data('Picking', true), false;
		})
		.on(('ontouchstart' in document.documentElement) ? 'touchend touchleave touchcancel' : 'mouseup mouseleave', function(e)
		{
			return $(this).data('Picking', false), false;
		})
		.on(('ontouchstart' in document.documentElement) ? 'touchmove' : 'mousemove', function(e)
		{
			if($(this).data('Picking'))
			{
				var off = $(this).offset(),
					...
					pixel = $this.ColorPicker.getImageData(x, y, 1, 1),
				...
				{
					$this.Brushes[i].Foreground = color;
				}
			}
	...
	for(var i in $this.Brushes)
	{
		if($this.Brushes[i].Icon)
		{
			$('<a href="#brush" class="' + ($this.Brushes[i].Icon) + ((i == 'Default') ? ' active' : '') + '" title="' + i + '"></a>')
				.data('brush', i)
				.appendTo('.brushes')
				.on('click', function()
				{
					...
					$this.Brush = $this.Brushes[$(this).data('brush')];
	...
	else if('ontouchstart' in document.documentElement)
	{
		$('.artboards')
			.on('touchend touchleave touchcancel', 'canvas.layer.active', function(e)
			{
				...
				if(!layer.Active || layer.Hidden) return false;
